...
const database = process.env.DB_DATABASE|| "blog_noticias"
...
const darLike = async (idUsuario, idPublicacion) => {
    let resultado = await pool.query("select * from like_dislikes where id_usuario = $1 and id_publicacion = $2", [idUsuario, idPublicacion])
    if(!resultado.rows[0]){
        await pool.query("INSERT INTO like_dislikes(islike, id_usuario, id_publicacion) values(true, $1, $2)", [idUsuario, idPublicacion])
    }else {
        if(resultado.rows[0].islike == true){
            await pool.query("UPDATE like_dislikes SET islike = false where id_usuario= $1 and id_publicacion = $2", [idUsuario, idPublicacion])
        }else if(resultado.rows[0].islike == false){
            await pool.query("UPDATE like_dislikes SET islike = null where id_usuario= $1 and id_publicacion = $2", [idUsuario, idPublicacion])
        }else {
            await pool.query("UPDATE like_dislikes SET islike = true where id_usuario= $1 and id_publicacion = $2", [idUsuario, idPublicacion])
        }
    }
    return true;
}
...
const cantidadLikeAndDislike = async(idPublicacion) => {
    let consulta = `
        select islike, count(*) from like_dislikes where id_publicacion = $1 and islike is not null
        group by islike
    `
...
const getLike = async (idUsuario, idPublicacion) => {
    let consulta = "select islike from like_dislikes where id_usuario = $1 and id_publicacion = $2;"
